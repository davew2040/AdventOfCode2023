...
namespace AdventOfCode2023.Day02
{
    internal class Day02 : IAocDay
    {
        ...
        public async Task Run()
        {
            ...
            var possibleGames = input.Where(g => GameIsPossible(g.Value));

            var sum = possibleGames.Sum(v => v.Key);
        ...
        private bool GameIsPossible(IEnumerable<Dictionary<CubeColor, int>> rounds)
        {
            foreach (var round in rounds)
            {
                foreach (var kvp in round)
                {
                    if (kvp.Value > BagTotals[kvp.Key])
                    {
                        return false;
                    }
                }
